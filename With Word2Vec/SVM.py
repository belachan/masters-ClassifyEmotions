...
def ingest():
    ...
    data = pd.read_csv('/Users/isabelaruizroque/Documentos/Mackenzie - Macbook/Mestrado/Dissertação/Bases/wang_datasetComDisgeSurprise-2.txt', encoding= "ISO-8859-1")
...
# Construindo o SVM
svm_class = SVC(gamma=0.001,C=100,kernel='rbf')
...
def plot_confusion_matrix(cm, classes,
    ...
    plt.ylabel('Positivo')
    plt.xlabel('Negativo')
    plt.savefig("SVM-MatrizConfusao-Exec_" + str(execucao)+ "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm))
...
for execucao in range(1,11):
    ...
    for train_index, test_index in cv.split(train_vecs_w2v, y):
        ...
        clf = svm_class.fit(X_tr, y_tr)
        # Predict do SVM
        print("PREDICT do SVM...")
        ...
        f = open("SVM-ClassificationReport-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    ...
    f = open("SVM-MatrizConfusao-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    simplejson.dump(aux_matrix, f)
    f.close()
        
    # Salvando a acurácia das pastas para cada execução
    f = open("SVM-ACC-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    simplejson.dump(acc_folder, f)
    f.close()
    
    # Salvando a medida-F média para cada execução
    f = open("SVM-Media-DesvioPadrao-FSCORE-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(f_score_folder), np.std(f_score_folder))), f)
    f.close()
    
    # Salvando a Precisão média para cada execução
    f = open("SVM-Media-DesvioPadrao-PRECISION-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(precision_folder), np.std(precision_folder))), f)
    f.close()
    
    # Salvando a acurácia média para cada execução
    f = open("SVM-Media-DesvioPadrao-ACC-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(acc_folder), np.std(acc_folder))), f)
    f.close()
    
    # Salvando o tempo de execução em um arquivo
    f = open("SVM-TempoDeExecucao-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
...
# Salvando a acurácia média para cada execução
f = open("SVM-ACURACIA-Media-DesvioPadrao-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(acc_folder), np.std(acc_folder))), f)
f.close()

# Salvando a medida-F média para cada execução
f = open("SVM-MEDIDAF-Media-DesvioPadrao-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(f_score_folder), np.std(f_score_folder))), f)
f.close()

# Salvando a precisão média para cada execução
f = open("SVM-PRECISION-Media-DesvioPadrao-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(precision_folder), np.std(precision_folder))), f)
f.close()
