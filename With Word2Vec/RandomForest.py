...
def plot_confusion_matrix(cm, classes,
    ...
    plt.ylabel('Positivo')
    plt.xlabel('Negativo')
    plt.savefig("RandomForest-MatrizConfusao-Exec_" + str(execucao)+ "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm))
...
for execucao in range(1,11):
    ...
    for train_index, test_index in cv.split(train_vecs_w2v, y):
        ...
        f = open("RandomForest-ClassificationReport-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    ...
    featureimp = featureimp.tolist()
    
    f = open("RandomForest-FeatureImportance-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    simplejson.dump(featureimp, f)
    f.close()
    ...
    f = open("RandomForest-MatrizConfusao-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    ...
    f = open("RandomForest-ACC-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    simplejson.dump(acc_folder, f)
    f.close()
    
    # Salvando a acurácia média para cada execução
    f = open("RandomForest-Media-DesvioPadrao-ACC-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(acc_folder), np.std(acc_folder))), f)
    f.close()
    
    # Salvando a medida-F média para cada execução
    f = open("RandomForest-Media-DesvioPadrao-FSCORE-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(f_score_folder), np.std(f_score_folder))), f)
    f.close()
    
    # Salvando a Precisão média para cada execução
    f = open("RandomForest-Media-DesvioPadrao-PRECISION-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
    simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(precision_folder), np.std(precision_folder))), f)
    f.close()
    
    # Salvando o tempo de execução em um arquivo
    f = open("RandomForest-TempoDeExecucao-Exec_" + str(execucao) + "-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
...
# Salvando a acurácia média para cada execução
f = open("RandomForest-ACC-Media-DesvioPadrao-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(acc_folder), np.std(acc_folder))), f)
f.close()


# Salvando a medida-F média para cada execução
f = open("RandomForest-MEDIDAF-Media-DesvioPadrao-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(f_score_folder), np.std(f_score_folder))), f)
f.close()

# Salvando a precisão média para cada execução
f = open("RandomForest-PRECISION-Media-DesvioPadrao-Size_" + str(size_dimensions) + "-Window_" + str(window_size) + "-Sg_" + str(sg_algorithm) + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(precision_folder), np.std(precision_folder))), f)
f.close()
