...
def plot_confusion_matrix(cm, classes,
    ...
    plt.ylabel('Positivo')
    plt.xlabel('Negativo')
    plt.savefig("RandomForest-MatrizConfusao-Exec_" + str(execucao))
...
for execucao in range(1,11):
    ...
    for train_index, test_index in cv.split(features, y):
        ...
        f = open("RandomForest-ClassificationReport-Exec_" + str(execucao) + ".txt", 'w')
    ...
    f = open("RandomForest-FeatureImportance-Exec_" + str(execucao) + ".txt", 'w')
    ...
    f = open("RandomForest-MatrizConfusao-Exec_" + str(execucao) + ".txt", 'w')
...
# Salvando a acurácia média para cada execução
f = open("RandomForest-ACC-Media-DesvioPadrao-Size_" + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(acc_folder), np.std(acc_folder))), f)
f.close()


# Salvando a medida-F média para cada execução
f = open("RandomForest-MEDIDAF-Media-DesvioPadrao-Size_" + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(f_score_folder), np.std(f_score_folder))), f)
f.close()

# Salvando a precisão média para cada execução
f = open("RandomForest-PRECISION-Media-DesvioPadrao-Size_" + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(precision_folder), np.std(precision_folder))), f)
f.close()
