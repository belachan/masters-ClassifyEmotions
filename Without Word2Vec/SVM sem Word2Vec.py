...
def plot_confusion_matrix(cm, classes,
    ...
    plt.ylabel('Positivo')
    plt.xlabel('Negativo')
    plt.savefig("SVM-MatrizConfusao-Exec_" + str(execucao))
...
#rf_class = SVM(n_estimators=10)
from sklearn.svm import SVC
from sklearn.model_selection import ShuffleSplit,GridSearchCV

# Construindo o SVM
rf_class = SVC(kernel='rbf',gamma=0.001, C=100)
...
#print("Construindo o Grid Search...")
## set the gridsearch using 3-fold cross validation and 'ROC Area Under the Curve' as the cross validation score. 
#grid = GridSearchCV(rf_class, param_grid=param_grid_SVM, cv=10,scoring='accuracy')

#print("Fazendo o FIT do Grid Search...")
#grid.fit(features, y)
...
for execucao in range(1,11):
    ...
    for train_index, test_index in cv.split(features, y):
        ...
        f = open("SVM-ClassificationReport-Exec_" + str(execucao) + ".txt", 'w')
    ...
    f = open("SVM-MatrizConfusao-Exec_" + str(execucao) + ".txt", 'w')
...
# Salvando a acurácia média para cada execução
f = open("SVM-ACC-Media-DesvioPadrao" + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(acc_folder), np.std(acc_folder))), f)
f.close()


# Salvando a medida-F média para cada execução
f = open("SVM-MEDIDAF-Media-DesvioPadrao" + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(f_score_folder), np.std(f_score_folder))), f)
f.close()

# Salvando a precisão média para cada execução
f = open("SVM-PRECISION-Media-DesvioPadrao" + ".txt", 'w')
simplejson.dump(("%.2f%% (+/- %.2f%%)" % (np.mean(precision_folder), np.std(precision_folder))), f)
f.close()
